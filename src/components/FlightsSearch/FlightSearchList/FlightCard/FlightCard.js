  ...
  return (
    <article className="flight-card">
      <div className="flight-card__header">
        <img src={carrierLogo} alt={flight.carrier.caption} className="flight-card__carrier-logo"/>
        <div className="flight-card__price">
          <p className="flight-card__price-count">{flight.price.totalFeeAndTaxes.amount} &#8381;</p>
          <p className="flight-card__price-info">Стоимость для одного взрослого пассажира</p>
        </div>
      </div>
        ...
        {flight.legs.map(leg => (
            <li className="flight-card__route-container">
              <div className="flight-card__route-title">
                <p>{leg.segments[0].departureCity.caption}, {leg.segments[0].departureAirport.caption} <span>({leg.segments[0].departureAirport.uid})</span></p>
              ...
              <p className="flight-card__route-carrier">Рейс выполняет: {flight.carrier.caption}</p>
            </li>
        ))}
      </ul>
      <button
